...
> 分类：中国节日
> 主题序号：5.5
> 场景文件：kik_中国节日_05_春节.png
> 热区文件：kik_中国节日_05_春节.json
...
### II. Scene Background
Create a complete 春节 Spring Festival family-celebration environment inside a cozy Montessori-style play world.
...
- A cozy traditional Chinese family entrance / living room celebrating Spring Festival
- Bright red doorway with vertical red couplets on both sides as background anchor
- Inverted gold "福" character on the door panel as background anchor
...
Scene atmosphere:
- Warm, festive, joyful, family-loving, lucky, clearly navigable, and positive
- Color palette suggestion: 中国红、暖金、米白、暖木棕
- Subtle depth-of-field: center sharper, far edges slightly softer
- Soft warm indoor light with red lantern glow accents
...
- Character outfits may use festive Chinese-New-Year touches (red knit sweater, red padded jacket, soft tang-style coat, mini cheongsam-inspired dress) but must stay child-friendly and modern-comfortable.
- Clothing colors should be bright but soft child-friendly colors: light blue, cream white, warm yellow, soft pink, mint green, coral orange, lavender purple, or fresh denim blue. Festive red is welcome here as long as the overall palette stays soft.
...
- [hóng bāo] / 红包 / Red Envelope (/rɛd ˈɛnvəloʊp/)
...
1. 红包 (hóng bāo / Red Envelope) — small stack of red-and-gold red envelopes on a tea tray as foreground hero.
...
- 红包 (small rectangular envelope) and 福字 (square red paper with gold 福) must look clearly different in shape and content; 红包 stack on tray, 福字 hung on door.
- 对联 (vertical strips with multiple gold characters) and 福字 (single big gold 福 on square red paper) must not be merged.
- 灯笼 (round red lantern) and 鞭炮 (cluster of red paper rolls) must look clearly different — lantern is one round body, firecracker is many thin rolls bundled.
...
Arrow precision map:
- 红包 arrow -> lands on the red envelope stack body.
...
- The scene must feel like a premium miniature toy diorama in a warm Chinese New Year home, not a product display board and not a dark cinematic scene.
- Firecrackers (鞭炮) must remain decorative-only with no flame, smoke, or explosion effect.
...
- [ ] 生成 PNG：`kik_中国节日_05_春节.png`
- [ ] 生成 JSON：`kik_中国节日_05_春节.json`
