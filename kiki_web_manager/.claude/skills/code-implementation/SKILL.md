...
在实现任何代码之前，必须遵循以下开发规范：

### 1. 新功能开发指南标准
**文档路径**: `doc/framework/新功能开发指南标准_20250916.md`

**核心要求**:
- ✅ 遵循 DDD 架构
- ✅ 使用 `RequestManager` 进行 API 请求
- ✅ 使用统一异常系统 (`ApiResponseException` + `ApiResponseHandler`)
- ✅ **所有用户可见的文本必须使用国际化（i18n）**
- ✅ Controller 使用 GetX
- ✅ 实体放在 `domain/entities/`
- ✅ Repository 接口放在 `domain/repositories/`
- ✅ Repository 实现放在 `data/repositories/`

### 2. 国际化（i18n）规范 🌍
**文档路径**: `doc/INTERNATIONALIZATION.md`

**强制要求**:
- ✅ **所有用户可见的文本必须使用 AppLocalizations**
- ✅ **必须在所有三个 .arb 文件中添加翻译**:
  - `lib/l10n/app_zh.arb` (中文简体)
  - `lib/l10n/app_en.arb` (英文)
  - `lib/l10n/app_zh_TW.arb` (中文繁体)
- ✅ **修改 .arb 文件后必须运行 `flutter gen-l10n`**
- ❌ **禁止硬编码任何中文、英文或其他语言文本**

**使用方法**:
```dart
// 1. 导入 AppLocalizations
import '../../generated/app_localizations.dart';

// 2. 在 Widget 中使用
final localizations = AppLocalizations.of(context)!;
Text(localizations.yourKey)

// 3. 带参数的文本
Text(localizations.itemsCount(12))  // "12个物品" 或 "12 items"

// 4. 在需要 BuildContext 的地方使用 Builder
Widget _buildContent() {
  return Builder(
    builder: (context) {
      final localizations = AppLocalizations.of(context)!;
      return Text(localizations.yourKey);
    },
  );
}
```
...
### 3. UI 设计规范
**文档路径**: `doc/ui/ui_design_specification.md`

**设计要求**:
- 主色调: #00C37D (主绿色), #3FD280 (辅助绿色)
- 圆角: 16-24px
- 触摸热区: 最小 44x44px
