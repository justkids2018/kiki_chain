...
allowed_regex='^(\./backups|\./scripts|\./scripts/deploy-release|\./kiki_server|\./kiki_server/database)$'

remote_dirs=$(ssh "$SERVER" "cd $REMOTE_DIR && find . -maxdepth 2 -mindepth 1 -type d | sort")

echo "$remote_dirs"

echo -e "${YELLOW}校验仅保留: backups / scripts / scripts/deploy-release / kiki_server/database${NC}"
