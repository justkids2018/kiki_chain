...
# 用法：./scripts/deploy/deploy-backend.sh [--no-sync]
...
echo -e "${CYAN}========================================${NC}"
echo -e "${CYAN}  Hi Kiki - 后端部署${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""

# 1. 同步代码
if [ "$SKIP_SYNC" = false ]; then
    echo -e "${YELLOW}[1/5] 同步代码...${NC}"
    bash "$SCRIPT_DIR/deploy-sync.sh"
    echo ""
else
    echo -e "${YELLOW}[1/5] 跳过代码同步 (--no-sync)${NC}"
fi
...
# 3. 重新构建后端
echo -e "${YELLOW}[3/5] 构建后端镜像 (这可能需要几分钟)...${NC}"
ssh $SERVER "cd $REMOTE_DIR && docker compose -f $COMPOSE_FILE build backend --no-cache" 2>&1
echo -e "${GREEN}  ✅ 后端构建完成${NC}"

# 4. 重启服务
echo -e "${YELLOW}[4/5] 重启后端服务...${NC}"
ssh $SERVER "cd $REMOTE_DIR && docker compose -f $COMPOSE_FILE up -d backend"
echo -e "${GREEN}  ✅ 后端已重启${NC}"

# 5. 验证
echo -e "${YELLOW}[5/5] 等待服务启动并验证...${NC}"
sleep 10
...
# 测试 API
API_TEST=$(ssh $SERVER "docker exec hikiki_nginx curl -s -o /dev/null -w '%{http_code}' http://backend:8001/api/v1/mobile/scene/categories" 2>/dev/null || echo "000")
if [ "$API_TEST" = "200" ]; then
    echo -e "${GREEN}  ✅ API 响应正常 (HTTP $API_TEST)${NC}"
else
    echo -e "${RED}  ⚠️  API 响应异常 (HTTP $API_TEST)，可能还在启动中${NC}"
fi
...
echo -e "${GREEN}========================================${NC}"
echo "  后端地址: https://mtrain.xyz/api"
