...
#   4. 重新构建并重启前端 Docker
#
# ⚠️  首次部署请先执行：./scripts/deploy-tencent/setup-server.sh
# 用法：./scripts/deploy-tencent/deploy-all.sh
# ============================================
...
# ── 3. 重建后端 ───────────────────────────────────────────
echo -e "${YELLOW}[3/4] 重建后端（Rust 编译中，约 5-10 分钟）...${NC}"
...
sleep 10
API=$(ssh $SERVER "curl -s -o /dev/null -w '%{http_code}' https://kiki.keepthinking.me/api/v1/mobile/scene/categories 2>/dev/null || echo '000'")
if [ "$API" = "200" ]; then
    echo -e "${GREEN}  ✅ API 正常 (HTTP $API)${NC}"
else
    echo -e "${YELLOW}  ⚠️  API 响应: HTTP $API（可能还在启动）${NC}"
fi
...
echo -e "  ✅ 部署完成"
echo -e "========================================${NC}"
echo "  主站:     https://kiki.keepthinking.me"
echo "  管理后台: https://kiki.keepthinking.me/admin"
echo "  API:      https://kiki.keepthinking.me/api"
