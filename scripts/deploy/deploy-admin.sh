...
# Hi Kiki - 管理后台部署
# 用途：重新构建并部署 Vue 管理后台
# 用法：./scripts/deploy/deploy-admin.sh [--no-sync]
#   --no-sync  跳过代码同步
# ============================================
...
if [ "$SKIP_SYNC" = false ]; then
    ...
    echo -e "${YELLOW}[1/4] 跳��代码同步 (--no-sync)${NC}"
fi
...
ssh $SERVER "cd $REMOTE_DIR && docker compose -f $COMPOSE_FILE up -d admin && docker compose -f $COMPOSE_FILE restart nginx"
