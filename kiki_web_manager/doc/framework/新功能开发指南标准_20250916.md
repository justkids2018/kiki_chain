...
- **所有用户可见的文本必须使用国际化（i18n）：支持中文简体、中文繁体、英文三种语言**。
...
### 核心原则
所有用户可见的文本必须使用国际化系统，支持中文简体（zh）、中文繁体（zh_TW）、英文（en）三种语言。

### 添加新的翻译文本

#### 1. 在 .arb 文件中添加翻译
在 `lib/l10n/app_en.arb` 中添加（英文）：
```json
{
  "yourKey": "Your Text",
  "@yourKey": {
...
在 `lib/l10n/app_zh.arb` 中添加（中文简体）：
```json
{
  "yourKey": "你的文本"
}
...
在 `lib/l10n/app_zh_TW.arb` 中添加（中文繁体）：
```json
{
  "yourKey": "你的文本"
}
...
在 `lib/l10n/app_zh_TW.arb` 中：
```json
{
  "itemsCount": "{count}個物品"
}
```
...
#### 使用带参数的翻译
```dart
Text(localizations.itemsCount(12))  // 显示 "12个物品" 或 "12 items"
```

#### 在需要 BuildContext 的地方使用 Builder
如果在 StatelessWidget 的方法中需要使用国际化，使用 Builder 包装：
```dart
...
### 国际化检查清单
- [ ] 所有用户可见的文本都使用了 AppLocalizations
- [ ] 在所有三个 .arb 文件中都添加了对应的翻译
- [ ] 运行了 `flutter gen-l10n` 重新生成代码
- [ ] 没有硬编码的中文、英文或其他语言文本
- [ ] 带参数的文本正确使用了 placeholders
...
- 依赖管理：在 `ServiceLocator` 或 `AppServices.setFeatureXRepository` 中注册 `IFeatureXRepository` 的默认实现，方便测试替代
...
- [ ] **所有用户可见的文本是否使用国际化（在 .arb 文件中定义，使用 AppLocalizations）**
- [ ] **是否在所有三个 .arb 文件（zh/en/zh_TW）中添加了对应的翻译**
- [ ] **修改 .arb 文件后是否运行了 `flutter gen-l10n` 重新生成代码**
...
2. 实体：`domain/entities/reset_password_response.dart`
